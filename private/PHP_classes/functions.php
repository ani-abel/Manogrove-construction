...
function constructCopyImagePath($path){
    $decide = (preg_match("/index\.php$/i", $_SERVER["PHP_SELF"]) ? "" : "../");
    return "{$decide}private/project_images/copies/".basename($path);
}

function constructMainImagePath($path){
    $decide = (preg_match("/index\.php$/i", $_SERVER["PHP_SELF"]) ? "" : "../");
    return "{$decide}private/project_images/".basename($path);
}